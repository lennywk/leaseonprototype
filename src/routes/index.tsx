...
import { units, states, cities, properties, unitTypes, type UnitType } from "@/data/units";
import heroMall from "@/assets/hero-mall.jpg";

export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Available Retail Spaces — GGP LeaseOn" },
      { name: "description", content: "Browse flexible retail spaces available for short-term lease in high-traffic GGP shopping centers across the U.S." },
      { property: "og:title", content: "Available Retail Spaces — GGP LeaseOn" },
      { property: "og:description", content: "Browse flexible retail spaces available for short-term lease across GGP shopping centers." },
    ],
  }),
  component: Landing,
});
      ...
      <section className="bg-card border-b border-border sticky top-0 z-10">
        <div className="container-page py-4">
          <div className="flex items-center justify-between mb-3">
            ...
            {hasFilters && (
              <button onClick={clearFilters} className="text-xs text-[#52595f] hover:underline font-body">
                Clear Filters
              </button>
            )}
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            <Select value={stateFilter} onValueChange={setStateFilter}>
              ...
              <SelectContent className="[&_[data-highlighted]]:bg-[#fdfbf8] [&_[data-highlighted]]:text-foreground">
