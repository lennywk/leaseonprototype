...
import { units, getPriceLabel, type Unit } from "@/data/units";
import { Button } from "@/components/ui/button";

export const Route = createFileRoute("/compare")({
  head: () => ({
    meta: [
      { title: "Compare Spaces — GGP LeaseOn" },
      { name: "description", content: "Compare retail spaces side-by-side across size, price, features, and location." },
    ],
  }),
  component: Compare,
});
...
function Compare() {
  ...
  const rows: { label: string; render: (u: Unit) => React.ReactNode }[] = [
    {
      label: "Image",
      render: (u) => (
                ...
                {selected.map((u) => (
                  <td key={u.id} className="py-4 px-3">
                    <Button className="w-full" size="sm" asChild>
                      <Link to="/inquiry/$id" params={{ id: u.id }}>Inquire</Link>
                    </Button>
                  </td>
                ))}
              </tr>
