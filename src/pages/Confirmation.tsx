          ...
          <div className="space-y-6">
            <div className="bg-card border border-border rounded-lg p-6">
              ...
              <div className="space-y-3 text-sm font-body">
                ...
                <Row label="Category" value={data.merchandiseCategory} />
                <Separator />
                <Row label="Space" value={`${data.unitName} (${data.unitType})`} />
                <Row label="Size" value={`${data.sqft.toLocaleString()} sq ft`} />
                <Row label="Term" value={`${data.leaseStartDate} – ${data.leaseEndDate} (${data.months} mo)`} />
              </div>
            </div>
...
const Row = ({ label, value }: { label: string; value: string }) => (
  <div className="flex">
    {label && <span className="text-muted-foreground w-32 flex-shrink-0 font-medium">{label}:</span>}
